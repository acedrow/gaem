#!/usr/bin/env node
// Rebuild @gaem/shared only when dist is missing or older than src.
// Avoids racing `tsc --watch` under `dev:cf` when e2e starts its stack.
import { spawnSync } from "node:child_process";
import { existsSync, readdirSync, statSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";

const rootDir = join(dirname(fileURLToPath(import.meta.url)), "../../..");
const sharedDir = join(rootDir, "packages/shared");
const distEntry = join(sharedDir, "dist/index.js");
const srcDir = join(sharedDir, "src");

function newestMtimeMs(dir) {
  let newest = 0;
  for (const name of readdirSync(dir, { withFileTypes: true })) {
    const path = join(dir, name.name);
    if (name.isDirectory()) {
      newest = Math.max(newest, newestMtimeMs(path));
    } else {
      newest = Math.max(newest, statSync(path).mtimeMs);
    }
  }
  return newest;
}

const needsBuild =
  !existsSync(distEntry) || newestMtimeMs(srcDir) > statSync(distEntry).mtimeMs;

if (!needsBuild) {
  console.log("[e2e] @gaem/shared dist is up to date — skipping rebuild");
  process.exit(0);
}

console.log("[e2e] building @gaem/shared…");
const result = spawnSync("npm", ["run", "build", "-w", "@gaem/shared"], {
  cwd: rootDir,
  stdio: "inherit",
  shell: process.platform === "win32",
});
process.exit(result.status ?? 1);
