...
import { buildBoardOccupancy, clampHp, getEnemyMaxHp, getPlayerMaxHp } from "../game.js";
...
function nearestUnoccupiedTile(
  ...
  while (queue.length) {
    ...
    for (const [dx, dy] of [
      ...
      const enemy = occ.enemyByKey.get(key);
      if (enemy && enemy.id !== excludeEnemyId) {
        queue.push({ x, y });
        continue;
      }
      return { x, y };
...
function pushOccupantOffTile(
  ...
  const enemy = occ.enemyByKey.get(key);
  if (enemy && enemy.id !== excludeEnemyId && !isTowerEnemy(enemy)) {
