...
import { buildSwarmGroups, reconcileSwarmHp, swarmGroupForEnemy, applySwarmMemberForcedMove } from "./swarm.js";
...
function isPlayerDestinationFree(
  ...
  if (enemy && enemy.id !== excludeEnemyId) return false;
  return true;
}
...
function occupantAt(
  ...
  const enemy = occ.enemyByKey.get(key);
  if (enemy && enemy.id !== excludeEnemyId) return { kind: "enemy", unit: enemy };
  return null;
}
