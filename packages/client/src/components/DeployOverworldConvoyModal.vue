...
const selectedType = ref<OverworldConvoyType>("supply");
const selectedFactionId = ref<FactionId>("syncrasis");

const FACTION_OPTIONS: { id: FactionId; name: string }[] = [
  { id: "syncrasis", name: "Syncrasis" },
  { id: "autophyes", name: "Autophyes" },
  { id: "paracletus", name: "Paracletus" },
];
...
watch(
  ...
  ([isOpen, qx]) => {
    if (!isOpen) return;
    selectedType.value = "supply";
    if (qx != null) {
      selectedFactionId.value = OVERWORLD_REGION_FACTIONS[regionIdForQuarter(qx)];
    }
  },
);
